  ...
  return (
    <div className="mt-8">
      <h3 className="text-lg font-medium text-white mb-4 flex items-center justify-between">
        ...
        <button 
          onClick={onSend}
          className="bg-primary-500 hover:bg-primary-600 text-white px-4 py-1.5 rounded-full text-sm transition-colors"
        >
          Send All
        </button>
      </h3>
      ...
      <div className="space-y-3 max-h-60 overflow-y-auto pr-2 custom-scrollbar">
        {Array.from(files).map((file, index) => (
          <div key={index} className="bg-white/5 border border-white/10 rounded-xl p-3 flex items-center gap-3">
